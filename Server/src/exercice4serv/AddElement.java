...
public class AddElement implements Command {
	...
	public Reference run(Reference receiver, SNode expr) {
		if(Execute.environment.references.containsKey(expr.get(0).contents()+"."+expr.get(2).contents())) {
			System.out.println("L'objet existe déja");
			return null;
		}
		//Récupération de la class de l'élément à créé
		Reference cls = Execute.environment.getReferenceByName(expr.get(3).get(0).contents());		
		...
		Execute.environment.addReference(expr.get(0).contents()+"."+expr.get(2).contents(), r);
		return r;
	}
